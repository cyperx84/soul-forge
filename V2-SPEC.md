...
7. `audit` fragment rules, `clone`, interview polish. **`review` ✅** (`internal/ingest/review.go` + `cmd/review.go`) — batches the judgment step by `(file, section, unresolved-axis-set)`; measured 7.6x collapse (280 per-axis decisions → 37). Emits a questionnaire, consumes answers, never decides. An unanswered batch is an error, not a skip; a stale answer (key matching no batch) is an error, or a decision made about different text tags live lines. **`merge` ✅** (`internal/ingest/merge.go` + `cmd/merge.go`) — the dedup step, shipped before `apply` per the ordering trap below; see the resolved finding for what it settled. **`audit` ✅** (`internal/audit` + `cmd/audit.go`) — the advisory pass: duplicate fragments (normalized text, emphasis-blind but backtick-respecting), pinned project state (dates are the strongest signal: doctrine is dateless, status is dated), vague language in rules, `--provenance` for uncited harness-behavior claims, narrow-scope candidates and byte budgets when `--targets` is given, and override reporting from profile chains. Exit 0/1/2 like `diff`. The audit/invariant line is failure cost: invariants corrupt output, findings rot — and promotion to invariant is one-way, since compile has no override flag by design. **`clone` ✅** (`cmd/clone.go` + `internal/fragment/profile.go`) — the provisioning primitive. Profile files (`{name, extends, fragments}`) with a relative extends path, so the tree survives being copied to the box it provisions (pinned by a relocation test). A clone starts empty: inheritance carries, nothing is copied — copying would be the hand-sync the tool exists to kill. `--set id=text` overrides an inherited fragment's text; `--retag id=axis:value` overrides a scope tag, because the E2E run found the placement wart immediately: a machine clone's override text is machine-specific, and the inherited `host:any` routed it into the portable rules file. Set+retag on one id emits one child fragment (two would be duplicate IDs, which compile rejects). Kind and lifecycle are deliberately not retaggable — changing them makes a different fragment, not a rescoped one. Every command's corpus loader now sniffs both formats (flat array from onboard/review, profile object from clone), so neither door's output is orphaned. Interview: shipped earlier as `review --interview` + `onboard` (migration and greenfield doors); no separate polish step remains.

The spec's `compile` command shipped as a library (`compile.Compile`) surfaced through `apply` (dry-run default, `--root` for sandboxes) and `diff` rather than as a third CLI verb that would render without comparing or writing — a preview that can't tell you what changed on disk is `diff` with less information.
