...
7. `audit` fragment rules, `clone`, interview polish. **`review` ✅** (`internal/ingest/review.go` + `cmd/review.go`) — batches the judgment step by `(file, section, unresolved-axis-set)`; measured 7.6x collapse (280 per-axis decisions → 37). Emits a questionnaire, consumes answers, never decides. An unanswered batch is an error, not a skip; a stale answer (key matching no batch) is an error, or a decision made about different text tags live lines. `audit`/`clone`/interview still open.

### Dedup is a review outcome, not a compile outcome (found 2026-07-15, unresolved)
