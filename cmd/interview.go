...
// the JSON by index.
//
// Everything user-specific in the brief comes from the run: --agents supplies the
// profile roster, --host the machine id, the ingested files supply the questions.
// The template itself names no person, agent, or machine — the tool has no user
// of its own, and a brief that hardcoded one would only ever work for its author.

// emitInterview renders the batches as a self-contained interviewer prompt.
func emitInterview(w io.Writer, ps []ingest.Proposal, batches []ingest.Batch, host string, agents []string) error {
	...
	profileValues := "`user`, `any`, or the agent's id"
	rosterNote := "The user's agent roster was not supplied to this run; accept any agent name the user says and record it lowercase."
	if len(agents) > 0 {
		quoted := make([]string, len(agents))
		for i, a := range agents {
			quoted[i] = "`" + a + "`"
		}
		profileValues = strings.Join(quoted, ", ") + ", `user`, or `any`"
		rosterNote = "That roster is this user's current fleet, not a closed set — if the user names an agent outside it, record the new name lowercase and flag the question `NEW_AGENT`."
	}

	hostValue := "`any` or a machine id"
	hostExample := "a machine id"
	if host != "" {
		hostValue = "`" + host + "` (the machine these files came from) or `any` (every machine)"
		hostExample = "`" + host + "`"
	}
	...
	fmt.Fprintf(w, `# Soul-Forge Tagging Interview — Voice Interviewer Brief

You are a voice interviewer in a live, full-duplex voice conversation with the user.
Your one job: walk them through %d tagging decisions about their AI-agent instruction
files, conversationally, with as little friction as possible, and capture every answer
exactly.
...
The tool ingested the user's real files and could not scope %d lines on its own.
Those are batched into the %d questions below (grouped by file section — one answer
tags the whole batch). Until these are answered, nothing can be compiled. **The
answers are the product of this conversation.**
...
- **host** — which machine the lines are true on.
  ...
  disk sizes, local paths → %s. Policies and habits → `+"`any`"+`.
...
- **kind** — what type of sentence it is.
  ...
  directive), `+"`voice`"+` (tone and speaking style), `+"`identity`"+` (an agent's name/role).
  This decides which file the line lands in when compiled.
...
1. Open by telling the user the shape: %d questions, grouped by file, roughly
   20–30 minutes, and they can say "skip" or "come back to that" anytime.
...
4. Accept natural speech and translate: "that's just for this machine" → host=%s.
   "everyone should know that" → profile=any. "that's about me, not the agents" →
   profile=user. "that's how [agent] talks" → profile=[agent], kind=voice. Confirm
   your translation back in a few words before moving on.
5. If the user gives a scope the axes can't express (e.g. "these two agents but not
   the others"), record their exact words in a note and flag the question
...
7. Batch momentum: consecutive questions often get the same answer. When you notice
   a pattern, offer "same as the last one?" — but still log each question
   individually.
...
Q2: host=%s profile=user kind=fact
Q3: skip
...
compiler's answer file, so exact axis names and lowercase values matter. At the end
of the conversation, produce the complete Q1–Q%d log in one block, plus the parking
lot.
...
`, bill.Batched, bill.Lines-bill.Resolved, bill.Batched,
		hostValue, hostExample, profileValues, rosterNote,
		bill.Batched, hostOr(host, "the machine id"), hostOr(host, "machine-id"),
		bill.Batched, bill.Batched, strings.Join(filesLine, ", "))
